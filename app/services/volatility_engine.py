...
class VolatilityEngine:
    ...
    @staticmethod
    def calculate_garch_vol(prices: List[float], period: int = 20, alpha: float = 0.1, beta: float = 0.8) -> float:
...
class KellyFraction:
    ...
    @staticmethod
    def estimate_ruin_probability(kelly_fraction: float, num_trades: int, win_rate: float) -> float:
...
class PositionSizer:
    ...
    @staticmethod
    def calculate_position_size(account_size: float, entry_price: float, stop_loss_price: float, win_rate: float, avg_win: float, avg_loss: float, current_vol: float, max_risk_per_trade: float = 0.02, kelly_fraction: float = 0.25) -> Dict:
        """Calculate position size using Kelly + volatility"""
        try:
            full_kelly = KellyFraction.calculate_kelly_full(win_rate=win_rate, avg_win=avg_win, avg_loss=avg_loss)
            fractional_kelly = KellyFraction.calculate_kelly_fractional(full_kelly=full_kelly, fraction=kelly_fraction)
            ...
            actual_risk = (position_size * price_distance) / account_size if account_size > 0 else 0
            ruin_prob = KellyFraction.estimate_ruin_probability(kelly_fraction=fractional_kelly, num_trades=100, win_rate=win_rate)
            return {"position_size": max(0, position_size), "position_pct": position_size / account_size if account_size > 0 else 0, "risk_pct": actual_risk, "kelly_fraction": fractional_kelly, "full_kelly": full_kelly, "vol_regime": vol_regime, "vol_current": current_vol, "ruin_prob_100_trades": ruin_prob, "kelly_win_rate": win_rate, "kelly_payoff_ratio": avg_win / avg_loss if avg_loss > 0 else 0}
        except Exception as e:
            print(f"Error in position sizing: {e}")
            return {"position_size": 0, "position_pct": 0, "risk_pct": 0, "kelly_fraction": 0, "full_kelly": 0, "vol_regime": "unknown", "vol_current": 0, "ruin_prob_100_trades": 0, "kelly_win_rate": 0, "kelly_payoff_ratio": 0}
